...
func NewRepository(client dbClient.Client, logger *logging.Logger) stat.Repository {
	return &repository{
		client: client,
		logger: *logger,
	}
...
// 	_, err := r.client.Query(ctx, query, &user.Email, &user.FullName, &user.Phone, &user.Role, &user.Id)
// 	if err != nil {
// 		return err
// 	}
